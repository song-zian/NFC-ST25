...
static void demoNfcv( rfalNfcvListenDevice *nfcvDev )
{
    ...
    uint16_t              rcvLen;
    uint8_t               blockNum = 9;
		uint8_t								numOfBlocks = 1;
	
    uint8_t               rxBuf[ 1 + DEMO_NFCV_BLOCK_LEN + RFAL_CRC_LEN ];                        /* Flags + Block Data + CRC */
    uint8_t *             uid; 
#if DEMO_NFCV_WRITE_TAG
    uint8_t               wrData[DEMO_NFCV_BLOCK_LEN] = { 0xff, 0x22, 0x33, 0x55 };             /* Write block example */
#endif
    ...
    */
    ...
    platformLog(" Read Block: %s %s\r\n", (err != ERR_NONE) ? "FAIL": "OK Data:", (err != ERR_NONE) ? "" : hex2Str( &rxBuf[1], DEMO_NFCV_BLOCK_LEN));
		...
		#if DEMO_NFCV_LOCK_BLOCK /* example of how to lock the block: Lock block only applicable for TV512 or TV02K */
				err = rfalNfcvPollerLockBlock(RFAL_NFCV_REQ_FLAG_DEFAULT,uid,blockNum);
				platformLog(" Lock Block %d: %s\r\n",blockNum,(err != ERR_NONE) ? "FAIL": "OK");
				/* check if the block is locked */
				err = rfalNfcvPollerGetBlockSecurityStatus( RFAL_NFCV_REQ_FLAG_DEFAULT, uid, blockNum, numOfBlocks,rxBuf, sizeof(rxBuf), &rcvLen);
//				platformLog(" Read Block status: %s\r\n", hex2Str( &rxBuf[1], numOfBlocks));
			
				if(rxBuf[1]==0)
				{
					platformLog(" Block %d is not locked.\r\n",blockNum );					
				}
				else
				{
					platformLog(" Block %d is locked!!\r\n",blockNum );		
				}
    #endif
}
